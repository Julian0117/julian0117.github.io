Shader "Custom/Clipping_Tubes_ColorFilter_Boost"
{
    Properties
    {
        ...
        _ColorBoost ("Color Boost (Saturation)", Range(0, 2)) = 1.0
        _BlackCutoff ("Black Cutoff (Brightness)", Range(0, 1)) = 0.05
        ...
        [Header(Clipping)]
        [Toggle] _clippingEnable("Enable Clipping", Int) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void geom(line Varyings input[2], inout TriangleStream<Varyings> triStream)
            {
                ...
                // Durchschnittsfarbe der Linie
                float4 c = (input[0].color + input[1].color) * 0.5;
                float luma = dot(c.rgb, float3(0.2126, 0.7152, 0.0722));

                
                // Black Cutoff (Farben nahe Schwarz filtern)
                if (luma < _BlackCutoff) return;
                
                // Farbe verstärken
                float3 boostedRGB = luma + (_ColorBoost + 1) * (c.rgb - luma);
                c.rgb = saturate(boostedRGB);

                
                // Ein Kanal ist dominant, wenn er größer ist als die anderen beiden 
                ...
                {
                    // Weder klar Rot, Grün noch Blau (z.B. Weiß, Schwarz oder Grau)
